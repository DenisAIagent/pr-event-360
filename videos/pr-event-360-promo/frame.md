...
description: >
  ...
  accent, the three-step gray text ladder, Inter Variable Variable (display/numerals/chrome) + Inter Variable (body),
...
colors:
  ...
  text-muted: "#0F7CB0"
  text-light: "#9A9A9A"
  accent-light: "rgba(21, 152, 211, 0.08)"
  accent-medium: "rgba(21, 152, 211, 0.15)"
  border: "rgba(21, 152, 211, 0.2)"
  card-bg: "rgba(21, 152, 211, 0.04)"
  positive: "#059669"
  negative: "#dc2626"
...
typography:
  # — reading ramp (Inter Variable body + Inter Variable Variable chrome) —
  body:    { fontFamily: "Inter Variable", cqw: 0.85, weight: 400, lineHeight: 1.6, color: "text-muted" }
  h4-eyebrow:{ fontFamily: "Inter Variable Variable", cqw: 0.8, weight: 600, tracking: "0.08em", upper: true, color: "primary" }
  tag:     { fontFamily: "Inter Variable Variable", px: 12, weight: 500, color: "primary" }
  counter: { fontFamily: "Inter Variable Variable", px: 13, weight: 500, tracking: "0.05em", color: "text-muted" }
  # — display / numerical ramp (Inter Variable Variable, near-black headings / cobalt numerals) —
  h3:      { fontFamily: "Inter Variable Variable", cqw: 1.25, weight: 500, lineHeight: 1.3, tracking: "-0.02em", color: "text" }
  stat-num:{ fontFamily: "Inter Variable Variable", cqw: 1.9, weight: 700, lineHeight: 1.0, color: "primary" }
  blockquote:{ fontFamily: "Inter Variable Variable", cqw: 2.4, weight: 500, lineHeight: 1.35, color: "text" }
  h2:      { fontFamily: "Inter Variable Variable", cqw: 2.6, weight: 600, lineHeight: 1.1, tracking: "-0.02em", color: "text" }
  metric-value:{ fontFamily: "Inter Variable Variable", cqw: 3.0, weight: 700, lineHeight: 1.0, color: "primary" }
  h1:      { fontFamily: "Inter Variable Variable", cqw: 4.2, weight: 700, lineHeight: 1.08, tracking: "-0.02em", color: "text" }
  quote-mark:{ fontFamily: "Inter Variable Variable", cqw: 8.0, weight: 700, lineHeight: 0.5, color: "primary", opacity: 0.15 }
...
components:
  ...
  cta-button:
    ...
    textColor: "{colors.bg}"
    rounded: "{radii.pill}"
    typography: "Inter Variable Variable 600"
...
- **Fonts** — already set to **Inter Variable** (display) / **Manrope Variable** (body); ignore any preset font name lingering in prose.
...
The voice is two faces in fixed roles: **Inter Variable Variable** (display, every numeral, all chrome —
eyebrows uppercase 0.08em) and **Inter Variable** (body, muted gray, line 1.6). Headlines are near-black;
...
- **Inter Variable Variable** (display/numerals/chrome) + **Inter Variable** (body) — near-black headlines, cobalt numerals.
...
Two ramps. The **reading ramp** (Inter Variable body 0.85cqw muted; Inter Variable Variable eyebrow uppercase 0.08em
cobalt) carries copy + chrome; the **display/numerical ramp** (Inter Variable Variable `h3` 1.25cqw → `h1`
...
- **Headlines near-black, −0.02em**; **eyebrows cobalt, uppercase, 0.08em**; **numerals cobalt 600–700**; **body Inter Variable 400 muted, line 1.6**. No italic, no uppercase body, no cobalt headline.
...
**Composes** accent-line, meta, h1, body sub. **Focal** a 2-line Inter Variable Variable `h1` near-black, left,
...
**Focal** a row of tinted cards — cobalt `metric-value` + Inter Variable label + muted desc + optional
...
blockquote, cite. **Focal** a Inter Variable Variable `blockquote` near-black under a 15%-opacity cobalt
...
Inter Variable body column beside a cobalt-8% highlight block (4px cobalt left rule) carrying an inline pull
...
a Inter Variable Variable `h1` near-black, centered, with the one solid cobalt `cta-button` pill below. **Accent**
...
- Use **tinted cards** (4% fill, 20% border, 10–14px radius, no shadow); body Inter Variable 400 muted, line 1.6.
...
- **Type** — Inter Variable Variable headings −0.02em near-black, cobalt eyebrows 0.08em + numerals; Inter Variable body muted line 1.6; ≥1.4cqw floor.
...
- **Inter Variable Variable + Inter Variable via Google Fonts.** CJK pairing (Noto Sans SC 700 display / Noto Serif SC 400 body) carries over; the eyebrow's uppercase+tracking signal weakens in CJK — pair it with the accent-line.
